...
const CharactersCard = ({name, height, mass, birthYear, gender}) => {
    return (
        <div className='character-card flex-col-sb-left flex-gap-12 shadow-default'>
            ...
            <div className='flex-row-sb-c flex-gap-15'>
                    {gender !== 'n/a' && <div className={`character-tag ${gender === 'male' ? 'character-tag-green' : gender === 'female' ? 'character-tag-pink' : gender === 'hermaphrodite' ? 'character-tag-yellow' : ''}`}>{gender}</div>}
